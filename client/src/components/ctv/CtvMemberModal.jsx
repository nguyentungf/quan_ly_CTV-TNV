...
export default function CtvMemberModal({
  isOpen,
  onClose,
  onSuccess,
  editingMember = null,
  ...
  useEffect(() => {
    if (editingMember) {
      ...
      const defaultGroup = (isLeader && user?.targetType === 'ctv' && user?.groupNum) ? Number(user.groupNum) : 1;
      setFormData({
        mssv: '',
        fullName: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingMember) {
        const res = await api.updateCtvMember(editingMember.id, formData);
        if (res.success) {
          addToast('Cập nhật thông tin CTV thành công!', 'success');
          onSuccess();
          onClose();
        } else {
          addToast(res.message || 'Lỗi khi cập nhật CTV', 'error');
        }
      } else {
        const res = await api.createCtvMember(formData);
        if (res.success) {
          addToast('Thêm mới CTV thành công!', 'success');
          onSuccess();
          onClose();
        } else {
          addToast(res.message || 'Lỗi khi thêm mới CTV', 'error');
        }
      }
    } catch (err) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden border border-slate-100 max-h-[92vh] flex flex-col">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-800">
                {editingMember ? 'Chỉnh Sửa Thông Tin CTV' : 'Thêm Mới Cộng Tác Viên'}
              </h3>
              <p className="text-xs text-slate-500">Phân cấp 8 nhóm quản lý</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-3.5 overflow-y-auto">
          <div className="grid grid-cols-2 gap-3.5">
            <div>
              ...
              <input
                ...
                placeholder="VD: 202619067"
                className={`w-full px-3 py-2 text-xs sm:text-sm border rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none ${
            ...
            <div>
              ...
              <input
                ...
                placeholder="VD: Vũ Thu Ngân"
                className="w-full px-3 py-2 text-xs sm:text-sm border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none"
                required
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">
                Nhóm (1 - 8) <span className="text-rose-500">*</span>
              </label>
              <select
                value={formData.groupNum}
                disabled={isLeader}
                onChange={(e) => setFormData({ ...formData, groupNum: Number(e.target.value) })}
                className={`w-full px-3 py-2 text-xs sm:text-sm border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none bg-white ${
          ...
          <div>
            ...
            <input
              ...
              placeholder="VD: Ngan.VT2619067@sis.hust.edu.vn"
              className="w-full px-3 py-2 text-xs sm:text-sm border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none"
            />
          </div>
          ...
          <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-slate-100">
            ...
            <button
              type="submit"
              disabled={loading}
              className="inline-flex items-center gap-1.5 px-5 py-2 rounded-xl text-xs sm:text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 shadow-sm shadow-blue-200 transition-all"
            >
